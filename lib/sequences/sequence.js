...
export default class Sequence
{
	...
	run( payload )
	{
		...
		if( this.checkConditions())
		{
			...
			this.parseCapturedPiece();

			if( this.xxx )
			{
				this.astNode.set( "value", this.result );
			}
		}
		else
		{
		...
		if( this.isOptional && this.result === failed )
		{
	...
	/**
	 * A condition that checks if the current stream value is equal
	 * to the target value.
	 *
	 * @param {string} target - The target value to compare with the current stream value.
	 * @returns {this}
	 */
	ifCurrent( target )
	{
		return this.if(({ stream }) =>
			Array.isArray( target )
				? target.includes( stream.current )
	...
	is( scopeKey, targetValue )
	{
		return this.if(({ scope }) =>
			Array.isArray( targetValue )
				? targetValue.includes( scope[ scopeKey ])
				: scope[ scopeKey ] === targetValue
		);
	}

	has( scopeKey )
	{
		return this.is( scopeKey, true );
	}

	not( scopeKey, targetValue )
	{
		return this.if(({ scope }) =>
			Array.isArray( targetValue )
				? ! targetValue.includes( scope[ scopeKey ])
	...
	parseCapturedPiece()
	{
		...
		const parser = new Parser({
			...
			offset: this.stream.cursor - this.result.length
		});
		const ast = parser.parse( this.result );

		this.astNode.set( this.astNodeName, [ ...ast ]);
	}
	...
	setupSubNode( value )
	{
		if( this.astNodeName && ! ( this.astNodeName in this.astNode.node ))
		{
			...
			// if we do not have a valid value, let's not add nodes
			// to the tree so that the tree will be a little lighter
